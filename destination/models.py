...
def get_image_upload_path(instance, filename):
    ...
    encoded_filename = hashlib.md5((filename_base + datetime.datetime.now().isoformat()).encode('utf-8')).hexdigest()

    # Convert location name to a suitable folder name
    location_name = unidecode(instance.location.name).replace(' ', '_')

    return 'images/location/{}/{}{}'.format(location_name, encoded_filename, filename_ext)
